...
from permutation_invariant.solutions_mpi_evojax import AttentionAgent
...
if __name__ == '__main__':
    ...
    for n_version in args.versions:
        for n_robo in args.ns_robo:
            log_dir = "log/aa_at{}/{}robo/".format(args.n_at, n_robo)
            # log_dir = "log/test_aa/"
            ...
            res_1 = subprocess.run("sh recode_aa_behave.sh {} {} {}".format(args.n_at, n_robo, n_version), shell=True)
            print("res_1:", res_1.returncode)
            res_2 = subprocess.run("sh recode_aa_attention.sh {} {} {}".format(args.n_at, n_robo, n_version), shell=True)
